...
class AnimationController {
    constructor(canvas, ctx, dpr, size) {
        ...
        this.viewZoom = 100;
        this.numberOfStars = 1500;  // Reduced from 5000 for performance
        this.trailLength = 40;       // Reduced from 80 for performance
    ...
    showProjectedDot(position, sizeFactor) {
        ...
        if (position.z > newCameraZ) {
            ...
            this.ctx.fillStyle = 'rgba(6, 182, 212, 0.8)'; // Make dots match cyan theme
    ...
    render() {
        ...
        // Transparent background so the underlying hero card glass effect shows through!
    ...
    drawTrail(t1) {
        for (let i = 0; i < this.trailLength; i++) {
            ...
            this.ctx.fillStyle = 'rgba(6, 182, 212, 1)'; // Cyan trail for theme mix
            this.ctx.lineWidth = sw;
...
export function SpiralAnimation() {
    ...
    useEffect(() => {
        const handleResize = () => {
            if (containerRef.current) {
                setDimensions({
                    // Add slight padding adjustments to keep the spiral centered nicely inside the hero card
        ...
        // Wait minor delay for accurate measurements just in case FramerMotion hasn't snapped dimension yet
    ...
    return (
        <div ref={containerRef} className="relative w-full h-full flex justify-center items-center overflow-hidden">
            <canvas ref={canvasRef} className="absolute blur-[0.5px] drop-shadow-[0_0_10px_rgba(6,182,212,0.8)] mix-blend-plus-lighter pointer-events-none" />
        </div>
    );
}
